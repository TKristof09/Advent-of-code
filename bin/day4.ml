...
let parse_card_wins line =
    ...
    let winning =
        List.filter_map (String.split winning ~on:' ') ~f:(fun x ->
            match x with "" -> None | s -> Some (Int.of_string s) )
    and picked =
        List.filter_map (String.split picked ~on:' ') ~f:(fun x ->
            match x with "" -> None | s -> Some (Int.of_string s) )
    in
    let num_common =
        Set.inter (Int.Set.of_list winning) (Int.Set.of_list picked)
        |> Set.length
    in
...
let part_two wins =
    ...
    let rec update_next num_won n cards =
        ...
        | [] ->
            []
        | x :: t ->
            if n > 0 then (x + num_won) :: update_next num_won (n - 1) t
            else cards
    in
    let rec aux l i =
        match l with
        | [] ->
            []
        | x :: t ->
            x :: (aux (update_next x wins.(i) t)) (i + 1)
    in
