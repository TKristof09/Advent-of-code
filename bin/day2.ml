...
type game = {id: int; sets: (cube * int) list list}
...
let parse_game s =
    ...
    let sets =
        List.map sets ~f:(fun set ->
            let cubes = String.split set ~on:',' in
            List.map cubes ~f:(fun cube ->
                let cube = String.drop_prefix cube 1 in
                (* drop space *)
                match String.lsplit2_exn cube ~on:' ' with
                | n, "blue" ->
                    (Blue, Int.of_string n)
                | n, "red" ->
                    (Red, Int.of_string n)
                | n, "green" ->
                    (Green, Int.of_string n)
                | _ ->
...
let calc_game_min g =
    ...
    and find_max_usages usages =
        List.fold usages ~init:(0, 0, 0) ~f:(fun (mr, mg, mb) (r, g, b) ->
...
let () =
    let lines = Aoc.read_to_list_filtered "day2" in
    List.fold lines ~init:0 ~f:(fun acc line ->
        let game = parse_game line in
