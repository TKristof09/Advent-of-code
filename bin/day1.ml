...
let part_one s =
    let digits =
        String.fold s ~init:[] ~f:(fun acc ch ->
            match Char.get_digit ch with None -> acc | Some x -> x :: acc )
    in
    match digits with
    | [] ->
        0
    | x :: [] ->
        (x * 10) + x
    | _ ->
        List.hd_exn digits + (List.last_exn digits * 10)
...
let part_two s =
    let rec aux l =
        ...
        | [] ->
            []
        | ch :: t when Char.is_digit ch ->
            Char.get_digit_exn ch :: aux t
        | 'o' :: 'n' :: 'e' :: _ ->
            1 :: aux (List.tl_exn l)
        | 't' :: 'w' :: 'o' :: _ ->
            2 :: aux (List.tl_exn l)
    ...
    | x :: [] ->
        (x * 10) + x
    | _ ->
        (List.hd_exn digits * 10) + List.last_exn digits
...
let () =
    ...
    let result =
        List.fold input_lines ~init:0 ~f:(fun acc line -> acc + part_two line)
    in
    Printf.printf "%d\n" result
