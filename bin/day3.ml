...
let parse_line line_num text =
    let is_part_number x y =
        let is_symbol ch =
            ...
            (* otherwise the <> opertator gets weird because it is shadowed by the <> on ints *)
    ...
    in
    let nums, last =
        String.foldi text.(line_num) ~init:([], []) ~f:(fun i (acc, cur) ch ->
          ...
          ~f:(fun i (acc, cur) ch ->
            if Char.is_digit ch then
              ...
              | num, gears ->
                  ...
                  let gear_map =
                      List.fold gears ~init:acc ~f:(fun acc' gears' ->
